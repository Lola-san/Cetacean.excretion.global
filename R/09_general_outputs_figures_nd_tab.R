...
#
# March 2022
# 09_general_outputs_figures_nd_tab.R
#
...
fig_exc_hab_taxa_log10 <- function(output_tib, 
  ...
  if (geo_area %in% c("West Indian ocean", "French Guyana", "French Antilles")) {
    color_vector <- c(`Deep divers` = "slategray3", 
                      `Small delphinids` = "#365579ff")
  } else {
    color_vector <- c(`Baleen whales` = "#cf7474ff", 
  ...
  output_tib |>
    ...
    #                  max = max(Excretion)) |>
    ggplot2::ggplot() +
    ggplot2::geom_boxplot(ggplot2::aes(x = Element, y = Excretion, fill = Eco_gp),  
    ...
    ggplot2::ylab("Nutrient release (in tons/yr)") +
    ggplot2::scale_y_continuous(trans = "log10") +
    ggplot2::ggtitle(geo_area) +
    ggplot2::xlab("") +
    #ggplot2::ylab("Nutrient release (in tons/km2/yr)") +
    ggplot2::theme_bw() +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5),
                   strip.text.x = ggplot2::element_text(face = "bold", size = 12),
                   strip.text.y = ggplot2::element_text(face = "bold", size = 12),
...
############## figures for SUMMER project report ###############################
